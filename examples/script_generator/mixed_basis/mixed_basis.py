...
# CFOUR input and runscript files for a HEAT-345(Q) 
# calculation
#
...
Usage:
    HEAT345q.py --name=<name> --ZMAT=<zmat> --runfile=<run.sh> 
    HEAT345q.py --joblist 
    HEAT345q.py --abrvs
    HEAT345q.py (-h | --help)
    HEAT345q.py --version
...
#
ZMAT_OPTIONS.update('H_basis' , Option(abrv = 'HYY', default = None      , description="Hydrogen Basis"        ) ) 
...
#        set_basis(BASIS.get(s), zopts)
        ...
        #set hydrogen basis
        H_name = s[2:] #this removes the aC part and just gives TZ, QZ, 5Z...?
        print("HNAME IS", H_name)
        zopts.set('H_basis', BASIS.get(H_name).GENBAS_name)  
        zopts.set('basis', BASIS.get(s).GENBAS_name)
...
if __name__ == '__main__':
    ...
    args = docopt(__doc__, version="HEAT345q 1.0")
